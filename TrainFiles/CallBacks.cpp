...
void curveTypeCB(Fl_Browser* widget, TrainWindow* tw)
{
	int value = widget->value();
	switch (value) {
	case 1:
		// linear
		tw->trainView->curveMode = CurveModeLinear;
		break;
	case 2:
		// cardinal cubic
		tw->trainView->curveMode = CurveModeCatmull;
		break;
	case 3:
		// cubic b spline
		tw->trainView->curveMode = CurveModeCubicBSplice;
	default:
		// not selected
		break;
	}
...
void trackTypeCB(Fl_Browser* widget, TrainWindow* tw)
{
	int value = widget->value();
	switch (value) {
	case 1:
		// normal
		tw->trainView->trackMode = TrackModeNormal;
		break;
	case 2:
		// double
		tw->trainView->trackMode = TrackModeDouble;
		break;
	default:
		// not selected
		break;
...
void fogButtonCB(TrainWindow* tw)
{
	tw->trainView->toggleFog();
}
